...
export default function Analytics() {
  ...
  const [todayStudyTimeSeconds, setTodayStudyTimeSeconds] = useState<number>(0);
  ...
  // Fetch today's study time from time_tracking table
  const fetchTodayStudyTime = async () => {
    ...
    try {
      ...
      // Sum all duration_seconds for today
  ...
  const fetchAnalyticsData = async (flushBuffer: boolean = false) => {
    if (!user?.id) {
    ...
    try {
      ...
      // Flush analytics buffer first if requested (for initial load or manual refresh)
      if (flushBuffer) {
        try {
          ...
          console.warn('⚠️ Failed to flush buffer (non-critical):', error);
        }
      }
      ...
      // Sync time_tracking to daily_analytics first (ensures weekly/monthly work)
      // This MUST happen before fetching analytics to ensure data is up-to-date
      try {
        ...
        console.log('🔄 Syncing time_tracking to daily_analytics...');
        const rollupResponse = await fetch(`${API_BASE_URL}/analytics/rollup/${user.id}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' }
        });
        
        if (!rollupResponse.ok) {
          throw new Error(`Rollup failed: ${rollupResponse.statusText}`);
        }
        
        // Wait a bit more after rollup to ensure database consistency
        await new Promise(resolve => setTimeout(resolve, 500));
        console.log('✅ Synced time_tracking to daily_analytics');
      } catch (error) {
        console.error('❌ Failed to sync time_tracking:', error);
        // Don't throw - continue with analytics fetch even if rollup fails
      }
      ...
      // Fetch today's study time from time_tracking table
      const todayTimeSeconds = await fetchTodayStudyTime();
      setTodayStudyTimeSeconds(todayTimeSeconds);
      ...
      // Get recent page visits from time_tracking table (more accurate)
      if (user?.id) {
        try {
          // Fetch from time_tracking table for accurate page visits
          const { data: timeTrackingData, error: trackingError } = await supabase
            .from('time_tracking')
            .select('page_type, start_time, end_time, duration_seconds')
            .eq('user_id', user.id)
            .order('start_time', { ascending: false })
            .limit(10);
          
          if (!trackingError && timeTrackingData) {
            // Map page types to display names
            const pageDisplayNames: Record<string, string> = {
              'topical_exam': 'Practice Mode',
            ...
            const visits = timeTrackingData.map(track => ({
              page: track.page_type,
              pageName: pageDisplayNames[track.page_type] || track.page_type || 'Unknown Page',
            ...
            // Fallback to buffer service
            const visits = analyticsBufferService.getRecentPageVisits(user.id, 10);
            setRecentPageVisits(visits);
          }
        } catch (error) {
          console.warn('⚠️ Error fetching recent page visits:', error);
          // Fallback to buffer service
          const visits = analyticsBufferService.getRecentPageVisits(user.id, 10);
          setRecentPageVisits(visits);
        }
      ...
      if (realTimeAnalytics) {
        ...
        // Use today's study time from time_tracking table (convert seconds to minutes)
        ...
        const validatedAnalytics = {
          ...
          today: {
            ...
            studyTimeMinutes: todayStudyTimeMinutes || 0,
  ...
  // Load data when user is available - automatically flush buffer on initial load
  useEffect(() => {
    if (user?.id) {
      // Small delay to ensure component is fully mounted
      const timer = setTimeout(() => {
        // Flush buffer and fetch fresh data on initial load
        console.log('🔄 Analytics page: Auto-refreshing on load...');
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Refetch analytics data when database changes (without flush to avoid loops)
      ...
      .on(
        ...
        (payload) => {
          console.log('⏱️ Real-time time tracking update received:', payload);
          // Refetch today's study time when time_tracking changes
          fetchTodayStudyTime().then(seconds => {
            setTodayStudyTimeSeconds(seconds);
            // Also update analytics data to reflect new time
            if (analyticsData) {
              setAnalyticsData({
                ...analyticsData,
                today: {
                  ...analyticsData.today,
                  studyTimeMinutes: Math.round(seconds / 60)
                }
              });
            }
            // Refresh recent page visits
            if (user?.id) {
              supabase
                .from('time_tracking')
                .select('page_type, start_time, end_time, duration_seconds')
  ...
  const handleRefresh = async () => {
    ...
    try {
      // Fetch fresh analytics data with buffer flush
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{safeNumber(analyticsData?.today?.totalActivities || 0)}</div>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="text-center p-4 bg-green-50 rounded-lg">
                    ...
                    <div className="text-sm text-green-700">{t.weeklyStudyTime}</div>
                        </div>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="text-center p-4 bg-blue-50 rounded-lg">
                    ...
                    <div className="text-sm text-blue-700">{t.monthlyStudyTime}</div>
                  </div>
                  <div className="text-center p-4 bg-purple-50 rounded-lg">
                    <div className="text-2xl font-bold text-purple-600">
          ...
          <TabsContent value="today" className="space-y-6">
            {!analyticsData ? (
              <div className="text-center py-12">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
                <p className="text-gray-600">Loading today's progress...</p>
              </div>
            ) : (
              <>
            {/* Daily Goal Progress */}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="text-lg">
                            {visit.page === 'ai-tutor' || visit.page === 'lessons' ? '💬' :
                             visit.page === 'practice' || visit.page === 'topical_exam' || visit.page === 'practice_mode' ? '📖' :
                             visit.page === 'flashcards' ? '🧠' :
                             visit.page === 'visual-learning' || visit.page === 'visual_learning' ? '🎥' :
                             visit.page === 'mock-exam-p1' || visit.page === 'mock_exam' ? '🏆' :
                             visit.page === 'mock-exam-p2' ? '🏆' : '📚'}
                          </div>
                          <div>
                            <p className="text-sm font-medium text-gray-900">
                              {visit.pageName || visit.page || 'Unknown Page'}
                            </p>
                            <p className="text-xs text-gray-500">
                              {visit.duration ? `${Math.round(visit.duration / 60)} min` : ''} • {new Date(visit.timestamp).toLocaleString()}
                            </p>
                          </div>
                        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2">
                <li>
                  <button 
                    onClick={() => setCurrentPage('mock-exam-subject-selection')}
                    className="text-sm text-gray-600 hover:text-imtehaan-primary transition-colors"
                  >
                    Mock Exams
                  </button>
