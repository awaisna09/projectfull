...
export function AITutorPage() {
  ...
  const loadChatFromHistory = (chatId: string) => {
    ...
    if (chat) {
      ...
      const lastAssistantMsg = sortedMessages.filter(m => m.role === 'assistant').pop();
      if (lastAssistantMsg) {
  ...
  const clearAllChatHistories = () => {
    // Clear all chat histories for all topics
    const keysToRemove: string[] = [];
    
    // Get all localStorage keys that start with 'ai_tutor_chat_history'
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith('ai_tutor_chat_history')) {
    ...
    // Show confirmation
    alert(`✅ Cleared ${keysToRemove.length} chat history entries!\n\nAll chat histories for all topics have been cleared.`);
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Send message to AI Tutor with conversation history
              ...
              <div className="text-white p-4" style={{ 
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    size="sm"
                    onClick={() => {
                      if (confirm('Are you sure you want to clear ALL chat histories for ALL topics? This cannot be undone.')) {
                        clearAllChatHistories();
                      }
                    }}
                    className="text-white border border-red-300/50 hover:bg-red-500/20"
