...
export function StudentDashboard() {
      ...
      const quickLoadDashboard = async () => {
        try {
          ...
          try {
            ...
            await fetch(`${API_BASE_URL}/analytics/rollup/${user.id}`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' }
            });
            console.log('✅ Dashboard: Synced time_tracking to daily_analytics');
          } catch (error) {
            console.warn('⚠️ Dashboard: Failed to sync time_tracking (non-critical):', error);
          }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('⏱️ Dashboard: Real-time time tracking update received:', payload);
          // Sync time_tracking to daily_analytics when new tracking record is created/updated
          const envUrl = import.meta.env.VITE_API_BASE_URL;
          const isLocalDev = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
          const baseUrl = isLocalDev 
            ? 'http://localhost:8000' 
          ...
          fetch(`${API_BASE_URL}/analytics/rollup/${user.id}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
          }).then(() => {
          ...
          }).catch(err => console.warn('Failed to sync time tracking:', err));
        }
      )
      .subscribe();
  ...
  const fetchLearningAnalytics = React.useCallback(async () => {
    ...
    try {
      ...
      // Sync time_tracking to daily_analytics first
      try {
        const envUrl = import.meta.env.VITE_API_BASE_URL;
        const isLocalDev = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
        const baseUrl = isLocalDev 
        ...
        console.warn('⚠️ Failed to sync time_tracking (non-critical):', error);
      }
  ...
  const calculateTotalTimeSpent = () => {
    // Use weekly time from daily_analytics (synced from time_tracking)
    if (dailyAnalytics?.thisWeek?.totalTimeSpent) {
      return dailyAnalytics.thisWeek.totalTimeSpent; // Already in seconds
    }
    return 0;
  };
  ...
  const getSubjectsForCurriculum = () => {
    ...
    const getSubjectAnalytics = (subjectName: string) => {
      ...
      // Time spent is now tracked via time_tracking system, not per-topic
      // Return 0 for topic-level time (not used in new system)
      const timeSpent = 0;
            ...
            <CardContent className="p-6">
              {loadingStudyPlans ? (
                ...
                <Button 
                  ...
                  <Button 
                      onClick={() => {
                        localStorage.setItem('createNewPlan', 'true');
                  ...
                  {(() => {
                    ...
                    return (
                      <div 
                        key={plan.plan_id} 
                        className="relative overflow-hidden bg-gray-50 rounded-xl p-6 border border-gray-200 hover:shadow-md transition-shadow cursor-pointer"
                        onClick={(e) => {
                          // Don't trigger if clicking delete button or its parent
                          if ((e.target as HTMLElement).closest('button[aria-label="Delete study plan"]')) {
                            return;
                          }
                          // Store selected plan ID and navigate to study planner
                          localStorage.setItem('selectedPlanId', plan.plan_id.toString());
                          localStorage.setItem('selectedSubjectId', (plan.subject_id || 101).toString());
                          setCurrentPage('study-plan');
                        }}
                      >
                      ...
                      return (
                        <div className="p-4 border rounded-lg hover:shadow-md transition-shadow cursor-pointer"
                          ...
                          <div className="text-sm text-gray-600 mb-2">
                            {topTopic.questionsAttempted} questions completed
                          </div>
                          <Button
                            size="sm"
                            variant="outline"
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="p-4 border rounded-lg hover:shadow-md transition-shadow cursor-pointer"
                          ...
                          <div className="text-sm text-gray-600 mb-2">
                            {reviewTopic.questionsAttempted} questions
                          </div>
                          <Button
                            size="sm"
                            variant="outline"
