...
export function FlashcardPage() {
  ...
  // Keep timeSpentRef in sync with stats.timeSpent
  useEffect(() => {
    timeSpentRef.current = stats.timeSpent;
  }, [stats.timeSpent]);
  ...
  useEffect(() => {
    return () => {
      ...
      if (currentUser?.id && stats.timeSpent > 0) {
        const unsavedTime = stats.timeSpent - savedTimeRef.current;
        if (unsavedTime > 0) {
          console.log(`🕒 FlashcardPage: Saving ${unsavedTime} seconds of unsaved study time`);
          comprehensiveAnalyticsService.addStudyTime(currentUser.id, unsavedTime, 'flashcards');
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const saveInterval = setInterval(() => {
      ...
      if (elapsedSinceLastSave >= 30) {
        console.log(`🕒 FlashcardPage: Periodic save - ${elapsedSinceLastSave} seconds`);
        comprehensiveAnalyticsService.addStudyTime(currentUser.id, elapsedSinceLastSave, 'flashcards');
      ...
      <div className="relative z-10 backdrop-blur-md border-b sticky top-0 bg-white/80" style={{ 
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            {/* Right: Time Tracking & Accuracy Stats */}
            <div className="flex items-center space-x-3">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border" style={{ 
                ...
                <div className="flex items-center gap-2">
                  <Clock className="h-4 w-4" style={{ color: palette.violet }} />
                  <span className="text-sm font-semibold text-black">
                    {formatTime(stats.timeSpent)}
                  </span>
                </div>
                <div className="h-4 w-px" style={{ backgroundColor: palette.violet }}></div>
                <div className="flex items-center gap-1.5">
                  {stats.timeSpent - savedTimeRef.current < 30 ? (
                    <>
                      <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                      <span className="text-xs text-black/70">Saved</span>
                    </>
                  ) : (
                    <>
                      <div className="w-2 h-2 bg-orange-500 rounded-full animate-pulse"></div>
                      <span className="text-xs text-black/70">Saving...</span>
                    </>
                  )}
                </div>
              </div>
