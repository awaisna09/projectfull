...
export class ComprehensiveAnalyticsService {
   ...
   */
  private async calculateRealAccuracy(userId: string, startDate: string, endDate: string): Promise<{
    questionsAttempted: number;
    questionsCorrect: number;
    accuracy: number;
  }> {
    ...
    try {
      ...
      console.log(`📊 Real Accuracy: ${totalCorrect}/${totalAttempted} = ${overallAccuracy}%`);
  ...
  async getTodayProgress(userId: string): Promise<DailyProgressData> {
    try {
      ...
      const accuracyData = await this.calculateRealAccuracy(userId, todayStart, todayEnd);
      ...
      if (dailyData && !dailyError) {
        ...
        console.log('📊 Mapped data with real accuracy - dailyAccuracy:', mappedData.dailyAccuracy);
        return mappedData;
      }

      // If no data in daily_analytics table, return empty progress with real accuracy
      console.log('📊 No daily analytics data found, returning empty progress with real accuracy');
  ...
  async getWeeklyProgress(userId: string): Promise<WeeklyProgressData> {
    try {
      ...
      return this.calculateWeeklyProgressWithAccuracy(userId, weeklyAnalytics);
    } catch (error) {
  ...
  private calculateWeeklyProgress(data: any[]): WeeklyProgressData {
    // This method is now deprecated, use calculateWeeklyProgressWithAccuracy
    return this.getEmptyWeeklyProgress();
  }
   ...
   */
  private async calculateWeeklyProgressWithAccuracy(userId: string, weeklyData: any[]): Promise<WeeklyProgressData> {
    try {
      const endDate = new Date();
      const startDate = new Date(endDate.getTime() - 7 * 24 * 60 * 60 * 1000);
      const weekStart = startDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
      ...
      const totalActivities = weeklyData.reduce((sum, day) => sum + (day.total_activities || 0), 0);
      const totalTimeSpent = weeklyData.reduce((sum, day) => sum + (day.total_time_spent || 0), 0);
      const studyStreak = weeklyData[weeklyData.length - 1]?.study_streak || 0;

      // Find most productive day
      let mostProductiveDay = 'No data';
      if (weeklyData.length > 0) {
        const mostProductiveData = weeklyData.reduce((max, day) => 
          (day.total_time_spent || 0) > (max.total_time_spent || 0) ? day : max
        );
        mostProductiveDay = new Date(mostProductiveData.date).toLocaleDateString('en-US', { weekday: 'long' });
      }
      ...
      console.error('❌ Error calculating weekly progress with accuracy:', error);
      return this.getEmptyWeeklyProgress();
    }
  }
   ...
   */
  private async calculateMonthlyProgressWithAccuracy(userId: string, monthlyData: any[]): Promise<MonthlyProgressData> {
    try {
      const endDate = new Date();
      const startDate = new Date(endDate.getTime() - 30 * 24 * 60 * 60 * 1000);
      const month = endDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
      ...
      const totalActivities = monthlyData.reduce((sum, day) => sum + (day.total_activities || 0), 0);
      const totalTimeSpent = monthlyData.reduce((sum, day) => sum + (day.total_time_spent || 0), 0);
      const longestStreak = Math.max(...monthlyData.map(day => day.study_streak || 0), 0);
      
      // Calculate study patterns
      const totalMinutes = Math.round(totalTimeSpent / 60);
      const daysWithData = monthlyData.filter(day => (day.total_time_spent || 0) > 0).length;
      const averageSessionLength = daysWithData > 0 ? Math.round(totalMinutes / daysWithData) : 0;
      ...
      const percentReady = Math.min(Math.round((accuracyData.accuracy + (totalMinutes / 18)) / 2), 100);
      const daysNeeded = Math.max(20 - Math.floor(totalMinutes / 90), 0);
      ...
      console.error('❌ Error calculating monthly progress with accuracy:', error);
  ...
  async getMonthlyProgress(userId: string): Promise<MonthlyProgressData> {
    try {
      ...
      if (monthlyData && monthlyData.length > 0) {
        return this.calculateMonthlyProgressWithAccuracy(userId, monthlyData);
      }

      // Fallback to enhanced tracker
      const monthlyAnalytics = await enhancedAnalyticsTracker.getMonthlyAnalytics(userId);
      return this.calculateMonthlyProgressWithAccuracy(userId, monthlyAnalytics);
    } catch (error) {
  ...
  async getRecentAchievements(userId: string): Promise<string[]> {
    try {
      ...
      const studyMinutes = Math.round((todayData.total_time_spent || 0) / 60);
      if (studyMinutes >= 120) {
        achievements.push('🏆 Ultra Studious: 2+ hours of focused study today!');
      } else if (studyMinutes >= 90) {
        achievements.push('⭐ Power Learner: 90+ minutes of study today!');
      } else if (studyMinutes >= 60) {
        achievements.push('✅ Daily Goal: Completed your 60-minute target!');
      } else if (studyMinutes >= 30) {
        achievements.push('📚 Half Way There: 30+ minutes completed!');
      }

      // Accuracy Achievements (from real performance)
      if (accuracyData.questionsAttempted >= 5) {
        if (accuracyData.accuracy >= 95) {
          achievements.push('🌟 Perfect Score: 95%+ accuracy achieved!');
        } else if (accuracyData.accuracy >= 90) {
          achievements.push('💎 Excellence: 90%+ accuracy on real tests!');
        } else if (accuracyData.accuracy >= 85) {
          achievements.push('🎯 High Achiever: 85%+ accuracy!');
        } else if (accuracyData.accuracy >= 75) {
          achievements.push('📈 On Track: 75%+ accuracy maintained!');
        }
      }

      // Question Practice Achievements
      if (accuracyData.questionsAttempted >= 50) {
        achievements.push('📝 Question Pro: 50+ questions attempted today!');
      } else if (accuracyData.questionsAttempted >= 25) {
        achievements.push('📝 Practice Warrior: 25+ questions attempted!');
      } else if (accuracyData.questionsAttempted >= 10) {
        achievements.push('📝 Question Master: 10+ questions attempted!');
      }

      // Activity Achievements
      if (todayData.lessons_completed >= 5) {
        achievements.push('🎓 Knowledge Seeker: 5+ lessons completed!');
      } else if (todayData.lessons_completed >= 3) {
        achievements.push('🎓 Lesson Learner: 3+ lessons completed!');
      }

      if (todayData.flashcards_reviewed >= 30) {
        achievements.push('🧠 Memory Master: 30+ flashcards reviewed!');
      } else if (todayData.flashcards_reviewed >= 15) {
        achievements.push('🧠 Flashcard Pro: 15+ flashcards reviewed!');
      }

      if (todayData.mock_exams_taken >= 2) {
        achievements.push('🏅 Exam Ready: 2+ mock exams completed!');
      } else if (todayData.mock_exams_taken >= 1) {
        achievements.push('🏅 Test Taker: Mock exam completed!');
      }

      // Session Achievements
      if (todayData.session_count >= 5) {
        achievements.push('💪 Super Focused: 5+ study sessions today!');
      } else if (todayData.session_count >= 3) {
        achievements.push('💪 Multi-Session: 3+ study sessions!');
      }
      ...
      if (todayData.productivity_score >= 90) {
        achievements.push('⚡ Peak Performance: 90+ productivity score!');
      } else if (todayData.productivity_score >= 80) {
        achievements.push('⚡ Productivity Pro: 80+ productivity!');
      }

      // AI Tutor Achievements
      if (todayData.ai_tutor_interactions >= 20) {
        achievements.push('💬 Curious Mind: 20+ AI tutor interactions!');
      } else if (todayData.ai_tutor_interactions >= 10) {
        achievements.push('💬 Active Learner: 10+ AI tutor questions!');
      }
  ...
  /**
   * Add study time for a user (now uses buffer system)
   */
  async addStudyTime(userId: string, timeInSeconds: number, page: string = 'general'): Promise<void> {
    try {
      ...
      console.log(`🕒 Buffering ${timeInSeconds} seconds of study time for user ${userId} on ${page}`);

      // Buffer the time tracking first (localStorage) - this ensures no data loss on refresh
      analyticsBufferService.bufferTimeTracking(userId, page, timeInSeconds);

      // The buffer service will handle syncing to Supabase automatically
      // We don't need to immediately update the database here
      // This prevents double-syncing and conflicts

      console.log(`✅ Buffered ${timeInSeconds}s - will sync automatically`);
  ...
  /**
   * DEPRECATED: Direct database update - now handled by buffer system
   * Kept for legacy compatibility if needed
   */
  private async addStudyTimeDirectly(userId: string, timeInSeconds: number): Promise<void> {
    try {
      const today = new Date().toISOString().split('T')[0];
      console.log(`🕒 LEGACY: Adding ${timeInSeconds} seconds directly to database`);
      ...
      console.log(`✅ LEGACY: Added ${timeInSeconds} seconds directly. Total: ${analytics.total_time_spent} seconds`);
    } catch (error) {
  ...
  async getStudyInsights(userId: string): Promise<{
    ...
    try {
      ...
      if (analytics.today.studyTimeMinutes === 0) {
        ...
        insights.push(`📚 Good start! You've studied for ${analytics.today.studyTimeMinutes} minutes today`);
        recommendations.push('Try to reach your 60-minute daily goal');
      } else if (analytics.today.studyTimeMinutes >= 60) {
        insights.push(`🏆 Excellent! You've achieved your daily goal (${analytics.today.studyTimeMinutes} minutes)`);
        recommendations.push('Consider challenging yourself with mock exams');
      } else {
        insights.push(`⏰ ${analytics.today.studyTimeMinutes} minutes completed - ${60 - analytics.today.studyTimeMinutes} minutes to goal`);
        recommendations.push('Keep up the momentum!');
      }

      // Accuracy Insights (from real performance data)
      if (analytics.today.questionsAttempted > 0) {
        if (analytics.today.dailyAccuracy < 60) {
          insights.push(`⚠️ Accuracy at ${analytics.today.dailyAccuracy}% - needs improvement`);
          recommendations.push('Review fundamentals and practice more questions');
          recommendations.push('Focus on understanding concepts, not memorizing');
        } else if (analytics.today.dailyAccuracy < 75) {
          insights.push(`📈 Accuracy at ${analytics.today.dailyAccuracy}% - making progress`);
          recommendations.push('Keep practicing to reach 85%+ target');
        } else if (analytics.today.dailyAccuracy < 90) {
          insights.push(`✅ Good accuracy at ${analytics.today.dailyAccuracy}%`);
          recommendations.push('You\'re on track! Try harder practice questions');
        } else {
          insights.push(`🌟 Outstanding ${analytics.today.dailyAccuracy}% accuracy!`);
          recommendations.push('Challenge yourself with advanced mock exams');
        }
      } else {
        recommendations.push('Answer practice questions to track your accuracy');
      }
      ...
      if (analytics.today.sessionCount === 0) {
        ...
        recommendations.push('Try multiple shorter sessions for better retention');
      } else if (analytics.today.sessionCount >= 3) {
        insights.push(`💪 Great! ${analytics.today.sessionCount} study sessions today`);
        recommendations.push('Spaced repetition is helping you learn better');
      }

      // Streak Insights
      if (analytics.currentStreak === 0) {
        insights.push('🔥 Start building your study streak');
        recommendations.push('Study at least 15 minutes daily to build consistency');
      } else if (analytics.currentStreak < 3) {
        insights.push(`🔥 ${analytics.currentStreak}-day streak - keep it going!`);
        recommendations.push('Study tomorrow to extend your streak');
      } else if (analytics.currentStreak >= 7) {
        insights.push(`🔥🔥🔥 Amazing ${analytics.currentStreak}-day streak!`);
        recommendations.push('You\'re building excellent study habits');
      } else {
        insights.push(`🔥 ${analytics.currentStreak}-day streak building`);
      ...
      if (analytics.thisMonth.examReadiness) {
        ...
        if (readiness.percentReady < 50) {
          insights.push(`📊 Exam readiness: ${readiness.percentReady}% - increase study time`);
          recommendations.push(`Study ${readiness.recommendedDailyTime} minutes daily to be ready in ${readiness.daysNeeded} days`);
        } else if (readiness.percentReady < 80) {
          insights.push(`📊 Exam readiness: ${readiness.percentReady}% - good progress`);
          recommendations.push(`${readiness.daysNeeded} more days of focused study needed`);
        } else {
          insights.push(`✅ Exam ready at ${readiness.percentReady}%!`);
          recommendations.push('Focus on revision and mock exam practice');
        }
      }
  ...
  private calculateNextMilestone(today: DailyProgressData): string {
    // Prioritize by what's most important for student success
    
    // Study time milestones (most important)
    if (today.studyTimeMinutes === 0) return '⏰ Start studying today - aim for 15 minutes';
    if (today.studyTimeMinutes < 15) return '⏰ Reach 15 minutes of study time today';
    if (today.studyTimeMinutes < 30) return '⏰ Study for 30 minutes today';
    if (today.studyTimeMinutes < 60) return '⏰ Complete your 60-minute daily goal';
    
    // Accuracy milestones (from real performance)
    ...
    if (today.totalActivities < 10) return '📚 Complete 10 activities for the day';
    if (today.mockExamsTaken === 0) return '📝 Take your first mock exam';
    
    // Streak milestones
    if (today.studyStreak < 3) return '🔥 Build a 3-day study streak';
    if (today.studyStreak < 7) return '🔥 Achieve a 7-day study streak';
    if (today.studyStreak < 14) return '🔥 Reach a 2-week study streak';
    if (today.studyStreak < 30) return '🔥 Build a 30-day mastery streak';
    
    // Excellence milestones
    if (today.studyTimeMinutes < 120) return '⭐ Study for 2 hours today - go extra!';
   ...
   * The buffer system handles daily resets automatically
   * 
   * Manual reset for admin/testing purposes only
   */
  public async manualResetDailyAnalytics(userId: string): Promise<void> {
    try {
      console.warn('⚠️ MANUAL RESET: This should only be used for admin/testing purposes');
      ...
      localStorage.setItem('imtehaan_time_buffer', JSON.stringify(allBuffers));
  ...
  public async resetAndGetFreshAnalytics(userId: string): Promise<RealTimeAnalytics> {
    try {
      ...
      // Just get fresh data without resetting
      // The buffer system handles everything automatically
