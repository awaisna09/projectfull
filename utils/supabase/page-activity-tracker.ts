...
export class PageActivityTracker {
  ...
  private async updateDailyAnalytics(session: PageSession): Promise<void> {
    try {
      ...
      // Time tracking is now handled by time_tracking table and rollup endpoint
      // Only update activity counts, not time (time comes from time_tracking → rollup)
      const { error } = await supabase
        .from('daily_analytics')
        .upsert({
          date: today,
          user_id: session.userId,
          // Don't update total_time_spent - it's synced from time_tracking via rollup
