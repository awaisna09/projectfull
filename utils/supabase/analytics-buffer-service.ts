...
class AnalyticsBufferService {
  ...
  public async syncToSupabase(): Promise<{ success: boolean; synced: number; failed: number }> {
    ...
    try {
      // 1. Time tracking is now handled by time_tracking table and rollup endpoint
      // Skip syncing time buffers to daily_analytics (old system)
      // The new system uses time_tracking → rollup → daily_analytics
      const timeBuffers = this.getAllTimeBuffers();
      const timeBufferKeys = Object.keys(timeBuffers);
      
      // Clear time buffers (they're no longer used for time tracking)
      // Time is now tracked via usePageTimer → time_tracking table
      if (timeBufferKeys.length > 0) {
        console.log(`⚠️ Clearing ${timeBufferKeys.length} old time buffers (using new time_tracking system)`);
        const emptyBuffers: Record<string, TimeTrackingBuffer> = {};
        localStorage.setItem(this.TIME_BUFFER_KEY, JSON.stringify(emptyBuffers));
      }
