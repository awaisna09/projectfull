...
export class LearningActivityTracker {
  ...
  async trackActivity(activity: Omit<LearningActivity, 'user_id' | 'session_id' | 'session_start' | 'session_end'>): Promise<string> {
    try {
      ...
      await this.logToStudyPlan(fullActivity);
  ...
  async logToStudyPlan(activity: LearningActivity): Promise<void> {
    try {
      ...
      const planActivityType = activityTypeMap[activity.activity_type];
      ...
      // Get subject_id from activity or topic
      // We need to find subject_id from topic_id
      ...
      if (!topic?.subject_id) return; // Can't determine subject
